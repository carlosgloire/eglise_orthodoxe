...
if (isset($_POST['add'])) {
    ...
    if (empty($filename) || empty($name) ) {
        ...
        $existing_diacre_query = $db->prepare("SELECT * FROM sous_diacres WHERE photo=:photo");
        $existing_diacre_query->execute(['photo' => $filename]);
        
        if ($existing_diacre_query->fetch()) {
            $error = "Ce sous diacre existe déjà";
        } else {
            $query = $db->prepare('INSERT INTO sous_diacres (photo, name) VALUES (:photo, :name)');
            if ($query->execute(['photo' => $filename, 'name' => $name])) {
                if (move_uploaded_file($tempname, $folder)) {
                    $success = "Sous diacre $name ajouté avec succès";
                } else {
