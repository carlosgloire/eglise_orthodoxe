...
if (isset($_POST['add'])) {
    ...
    $folder = "../templates/metropolites_images/" . $filename;
    ...
    if (empty($filename) || empty($name)) {
        ...
        $existing_product_query = $db->prepare("SELECT * FROM metropolites WHERE photo=:photo");
        $existing_product_query->execute(array('photo' => $filename));
        $existing_product = $existing_product_query->fetch(PDO::FETCH_ASSOC);
        
        if ($existing_product) {
            $error = "Ce métropolites existe déjà";
        } else {
            $query = $db->prepare('INSERT INTO metropolites (photo,name) VALUES (:photo,:name)');
            ...
            if ($query->execute()) {
                ...
                if (move_uploaded_file($tempname, $folder)) {
                    $success = "Métropolites ajouté avec succès";
                } else {
                    $error = "Échec du téléchargement de l'image";
                }
            } else {
                $error = "Erreur lors de l'ajout de du métropolite";
            }
        }
    }
}
